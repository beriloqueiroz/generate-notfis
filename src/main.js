...
function includeEspacosVazios(valor, size) {
    let str = "";
...
async function main() {
    ...
    const listFiles = await listFilesInDiretory(pathDiretory)
    listFiles.forEach(elem => console.log(elem))
    await separateNfeXmlFilesByRegion(pathDiretory, 9);
